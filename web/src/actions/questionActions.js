const URL_BASE = 'https://question-back.herokuapp.com';
...
export function fetchQuestion(id) {
    return async dispatch => {
        dispatch(loading())
        try {
            const response = await fetch(`${URL_BASE}/quest/${id}`)
            const data = await response.json()
...
export function postAnswer(answer) {
    return async dispatch => {
        dispatch(loading())
        try {
            const response = await fetch(`${URL_BASE}/c-answer`,
                {
                    method: 'POST',
                    mode: 'cors',
                    headers: {
